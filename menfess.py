...
def add_group_to_db(chat_id: int, admin_id: int, title: str, link: str, chat_type: str):
    ...
    cursor = conn.cursor()
    cursor.execute("""
        REPLACE INTO groups (chat_id, admin_id, title, link, type, join_time) 
...
def create_backup_and_send_to_owner():
    try:
        # Ambil semua data grup dari database
        conn = get_db_connection()
        cursor = conn.cursor()
        ...
        # Buat file backup JSON
        backup_data = {
            "groups": [],
            "backup_time": datetime.now().isoformat()
        }

        # Konversi data grup ke format JSON
        for group in groups_data:
            ...
            backup_data["groups"].append(group_dict)

        # Simpan data JSON ke file
        json_file = "backup_data.json"
        with open(json_file, "w") as f:
            json.dump(backup_data, f, indent=4)

        # Buat file zip yang berisi database dan JSON
        ...
        app.send_document(
            chat_id=owner_id,
            document=backup_zip,
            caption=f"Database backup terbaru\nJumlah grup: {len(groups_data)}\nWaktu backup: {backup_data['backup_time']}"
        )
...
def restore_backup():
    try:
        ...
        json_file = "temp_restore/backup_data.json"
        ...
        if message.reply_to_message and message.reply_to_message.document:
            ...
            if file_name.endswith(".zip"):
                try:
                    ...
                    client.send_message(
                        chat_id=owner_id,
                        text=f"Database berhasil dipulihkan dari backup.\nJumlah grup: {len(backup_data['groups'])}"
                    )
